...
MD_PATH = "2026-07-06-AI-Daily.md"
HTML_PATH = "2026-07-06-AI-Daily.html"
...
def parse_news_items(md_text):
    ...
    每个条目的内容由连续的 '>' 引用块组成（其间可有空行），直到下一个 '> **🔹' 或非引用内容。
    """
...
def split_news_by_module(md_text):
    """根据 ## 模块标题，把新闻条目分到国际/国内两组。"""
    lines = md_text.splitlines()
    intl_items = []
    cn_items = []
    section = None
    ...
    for line in lines:
        if line.startswith("## ") and "国际动态" in line:
            section = "intl"
            continue
        if line.startswith("## ") and "国内动态" in line:
            # 把之前国际模块的条目提交
            intl_items.extend(cur_items)
            cur_items = []
            section = "cn"
            continue
        if line.startswith("## ") and "今日趋势点评" in line:
            if section == "intl":
                intl_items.extend(cur_items)
            elif section == "cn":
                cn_items.extend(cur_items)
            cur_items = []
            section = None
            break
        buf.append((section, line))
    # 解析整段，再按 section 归类
    return intl_items, cn_items


def build_body(md_text):
    items_all = parse_news_items(md_text)
    # 按模块归属：重新扫描确定每个条目所属模块
    lines = md_text.splitlines()
    section = None
    item_idx = 0
    intl_cards = []
    ...
    # 构建 body：手工组装，避免 blockquote 合并问题
    ...
    for li, line in enumerate(lines):
        ...
        if s.startswith("# "):
            ...
            if content == "":
                continue
            if ("维度" in content and "暂无" in content) or "弹性原则" in content or "本简报覆盖" in content:
...
<title>AI 行业每日热点深度简报 · 2026-07-06</title>
<style>{css}</style>
</head>
<body>
<div class="container">
